...
class RungeKuttaNystrom:
  ...
  def solve(self):
    ...
    t0_moment = {'tempo': t, 'deslocamento': y, 'velocidade': dy, 'aceleracao': self.f(t, y, dy)} 
    moments = [t0_moment]
    for _ in range(self.number_of_steps):
      K1 = half_step * self.f(t, y, dy)
      Q = half_step * (dy+1/2*K1)
      K2 = half_step * self.f(t+half_step, y+Q, dy+K1)
      K3 = half_step * self.f(t+half_step, y+Q, dy+K2)
      L = self.step*(dy+K3)
      K4 = half_step * self.f(t+self.step, y+L, dy+2*K3)

      # now increment Y, Y' and T
      y += self.step*(dy+(1/3)*(K1+K2+K3))
      dy += 1/3*(K1+2*K2+2*K3+K4)
      t += self.step
      moments.append({'tempo': t, 'deslocamento': y, 'velocidade': dy, 'aceleracao': self.f(t, y, dy)})
